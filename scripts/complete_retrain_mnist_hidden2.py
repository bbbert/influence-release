...
def get_originals(loss_full, correct, predicted_loss_diffs, first_seed, num_seeds):
    ...
    if refresh or not os.path.exists(path_name):
        for seed in range(first_seed,first_seed+num_seeds):
            ...
            if refresh or not os.path.exists("{}/{}-checkpoint-{}.meta".format(train_dir, model_name, iter_to_load)):
            ...
            predicted_loss_diffs[seed] = model.get_influence_on_test_loss([test_idx],
...
def complete_retrain(loss_remove_one, actual_retrain_loss_diffs, loss_full):
    for seed in seeds:
        ...
        for point in top_points:
            ...
            model_name = 'mnist_small_all_cnn_c_hidden2_seed{}_iter-{}_remove_{}'.format(seed, num_steps, point)
...
loss_full, correct, predicted_loss_diffs = get_originals(loss_full, correct, predicted_loss_diffs, first_seed, num_seeds)
