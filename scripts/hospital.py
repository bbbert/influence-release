...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Explore group influence in the hospital dataset')
    # Environment args
    ...
    parser.add_argument('--num-perturb', default=50, type=int,
                        help="The number of subsets to perturb")
    ...
    config = {
        ...
        'normalized_cross_validation_range': {
            ...
            'hospital_preprocess': (1e-4, 1e-1, 10),
        }[args.dataset_id],
        'inverse_hvp_method': 'explicit',
        'max_memory': int(args.max_memory),
    }

    exp = HospitalGroups(config, out_dir=args.out_dir)
