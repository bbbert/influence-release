...
force_refresh=True
train_dir = '../output'
regpathname = '{}/seed{}_losses_logreg'.format(train_dir,seed)
test_idx = 6558
num_steps = 300000 ########

print("Starting seed{}".format(seed))
tf.reset_default_graph()
model_name = 'mnist_small_all_cnn_c_hidden0_seed{}_iter-{}'.format(seed,num_steps)
...
np.savez('{}/seed{}_actual_pred_infl_logreg_remove_{}'.format(train_dir,seed,indices_to_remove),
        actual_infl=actual_loss_diffs,
...
for point in indices_to_remove:
    rempathname = '{}/seed{}_remove{}_only_losses_logreg'.format(train_dir,seed,point)
    print("Starting seed{} removing {}".format(seed,point))
    tf.reset_default_graph()
    model_name = 'mnist_small_all_cnn_c_hidden0_seed{}_iter-{}_remove_{}'.format(seed,num_steps,point)
    model = All_CNN_C(load_config(model_name))
